    ...
    for (i = os->nb_fragments - 1; i >= 0; i--) {
        ...
        if (offset >= frag->start_pos && offset < frag->start_pos + frag->size) {
            ...
            ret = ffurl_open_whitelist(&os->out, frag->file, AVIO_FLAG_WRITE,
                                       &os->ctx->interrupt_callback, &os->ctx->open_callback,
            ...
            ffurl_open_whitelist(&os->out2, frag->infofile, AVIO_FLAG_WRITE,
                                 &os->ctx->interrupt_callback, &os->ctx->open_callback, &opts, os->ctx->protocol_whitelist, os->ctx->protocol_blacklist, NULL);
            av_dict_free(&opts);
...
static int ism_flush(AVFormatContext *s, int final)
{
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        ret = ffurl_open_whitelist(&os->out, filename, AVIO_FLAG_WRITE, &s->interrupt_callback, &s->open_callback, NULL, s->protocol_whitelist, s->protocol_blacklist, NULL);
        if (ret < 0)
            break;
