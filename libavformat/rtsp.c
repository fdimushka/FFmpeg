        ...
        if (lower_transport == RTSP_LOWER_TRANSPORT_UDP) {
            ...
            while (j + 1 <= rt->rtp_port_max) {
                ...
                err = ffurl_open_whitelist(&rtsp_st->rtp_handle, buf, AVIO_FLAG_READ_WRITE,
                                 &s->interrupt_callback, &s->find_protocol_callback, &opts, s->protocol_whitelist, s->protocol_blacklist, NULL);
...
redirect:
    ...
    if (rt->control_transport == RTSP_MODE_TUNNEL) {
        ...
        if (ffurl_alloc(&rt->rtsp_hd, httpname, AVIO_FLAG_READ,
                        &s->interrupt_callback, &s->find_protocol_callback) < 0) {
            err = AVERROR(EIO);
            goto fail;
        }
        ...
        if ((ret = ffurl_open_whitelist(&rt->rtsp_hd, tcpname, AVIO_FLAG_READ_WRITE,
                       &s->interrupt_callback, &s->find_protocol_callback, NULL, s->protocol_whitelist, s->protocol_blacklist, NULL)) < 0) {
            err = ret;
            goto fail;
        }
